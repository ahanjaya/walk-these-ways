...
class CoRLRewards:
    ...
    def _reward_dof_pos_limits(self):
        ...
        out_of_limits = -(self.env.dof_pos - self.env.dof_pos_limits[:, 0]).clip(
            max=0.0
        )  # lower limit
        out_of_limits += (self.env.dof_pos - self.env.dof_pos_limits[:, 1]).clip(
            min=0.0
        )
    ...
    def _reward_jump(self):
        ...
        jump_height_target = (
            self.env.commands[:, 3] + self.env.cfg.rewards.base_height_target
        )
    ...
    def _reward_tracking_contacts_shaped_force(self):
        ...
        for i in range(4):
            reward += -(1 - desired_contact[:, i]) * (
                1
                - torch.exp(
                    -1 * foot_forces[:, i] ** 2 / self.env.cfg.rewards.gait_force_sigma
                )
            )
        return reward / 4
    ...
    def _reward_feet_slip(self):
        ...
        foot_velocities = torch.square(
            torch.norm(self.env.foot_velocities[:, :, 0:2], dim=2).view(
                self.env.num_envs, -1
            )
        )
    ...
    def _reward_feet_contact_vel(self):
        ...
        foot_velocities = torch.square(
            torch.norm(self.env.foot_velocities[:, :, 0:3], dim=2).view(
                self.env.num_envs, -1
            )
        )
    ...
    def _reward_feet_contact_forces(self):
        ...
        return torch.sum(
            (
                torch.norm(self.env.contact_forces[:, self.env.feet_indices, :], dim=-1)
                - self.env.cfg.rewards.max_contact_force
            ).clip(min=0.0),
            dim=1,
        )
    ...
    def _reward_feet_clearance_cmd_linear(self):
        ...
        for i in range(4):
            ...
            rew_foot_clearance = torch.square(target_height - foot_height) * (
                1 - self.env.desired_contact_states[:, i]
            )
    ...
    def _reward_feet_impact_vel(self):
        prev_foot_velocities = self.env.prev_foot_velocities[:, :, 2].view(
            self.env.num_envs, -1
        )
        contact_states = (
            torch.norm(self.env.contact_forces[:, self.env.feet_indices, :], dim=-1)
        ...
        rew_foot_impact_vel = contact_states * torch.square(
            torch.clip(prev_foot_velocities, -100, 0)
        )
    ...
    def _reward_orientation_control(self):
        ...
        desired_projected_gravity = quat_rotate_inverse(
            desired_base_quat, self.env.gravity_vec
        )
