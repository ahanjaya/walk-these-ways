...
class LeggedRobot(BaseTask):
    ...
    def step(self, actions):
        ...
        if self.feasibility_obs_buf is not None:
            self.feasibility_obs_buf = torch.clip(
                self.feasibility_obs_buf, -clip_obs, clip_obs
            )
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.commands.command_curriculum:
            ...
            for curriculum, category in zip(self.curricula, self.category_names):
                self.extras[f"curriculum/distribution"][
                    f"weights_{category}"
                ] = curriculum.weights
                self.extras[f"curriculum/distribution"][
                    f"grid_{category}"
    ...
    def compute_observations(self):
        ...
        if self.cfg.env.priv_observe_ground_friction:
            ...
            ground_friction_coeffs_scale, ground_friction_coeffs_shift = (
    ...
    def _reset_root_states(self, env_ids, cfg):
        ...
        quat = quat_from_angle_axis(init_yaws, torch.Tensor([0, 0, 1]).to(self.device))[
            :, 0, :
        ]
    ...
    def _init_buffers(self):
        ...
        if self.cfg.terrain.measure_heights:
            ...
            self.height_points_around_foot, self.num_height_points_around_foot = (
                self._init_height_points(
                    torch.arange(self.num_envs, device=self.device),
    ...
    def _init_height_points(self, env_ids, measured_points_x, measured_points_y):
        ...
        points = torch.zeros(
            len(env_ids), num_height_points, 3, device=self.device, requires_grad=False
        )
    ...
    def _get_heights(
        ...
        if cfg.terrain.mesh_type == "plane":
            return torch.zeros(
                len(env_ids), num_height_points, device=self.device, requires_grad=False
            )
