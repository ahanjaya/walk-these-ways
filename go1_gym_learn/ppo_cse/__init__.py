...
class Runner:
    ...
    def learn(
        ...
        )  # TODO: check, is this correct on the first step?
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    obs_history = obs_dict["obs_history"].to(self.device)
                    feasibility_obs = obs_dict["feasibility_obs"].to(self.device)
                    feasibility_targets = obs_dict["feasibility_targets"].to(
                        self.device
                    )
                ...
                if it % curriculum_dump_freq == 0:
                    logger.save_pkl(
                        ...
                        path=f"curriculum/info.pkl",
                        append=True,
                    )

                    if "curriculum/distribution" in infos:
                        logger.save_pkl(
                            {"iteration": it, "distribution": distribution},
                            path=f"curriculum/distribution.pkl",
                            append=True,
                        )
            ...
            if logger.every(RunnerArgs.log_freq, "iteration", start_on=1):
                ...
                logger.log_metrics_summary(
                    key_values={"timesteps": self.tot_timesteps, "iterations": it}
                )
                logger.job_running()
